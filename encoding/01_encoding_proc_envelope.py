...
if __name__ == "__main__":
    ...
    save_path = os.path.join(repository, 'derivatives', 'encoding', 'encoding_mel_hilb')
    ...
    for wav_file in wav_files:
        ...
        zband_envelope= np.abs(hilbert(zscore_melspec, axis=0))
        ...
        outfile_base = os.path.basename(wav_file).split('.')[0][17:] 
        dict_sr[outfile_base] = sr_spec
        dict_freq[outfile_base] = freqs
        with open(os.path.join(save_path,'envelope_hilb_z_score_'+ outfile_base +'.pkl'), 'wb') as f:
            pickle.dump(mean_zband_envelope, f)
        with open(os.path.join(save_path,'envelope_hilb_'+ outfile_base +'.pkl'), 'wb') as f:
            pickle.dump(mean_band_envelope, f)
            
    with open(os.path.join(save_path_meta,'dict_sr.pkl'), 'wb') as f:
        pickle.dump(dict_sr, f)
    with open(os.path.join(save_path_meta,'dict_freq.pkl'), 'wb') as f:
        pickle.dump(dict_freq, f)
    ...
    #np.savetxt(tsv_file, mean_zband_envelope, delimiter='\t')
    #metadata = {'SamplingFrequency': sr_spec, 'StartTime': 0,
    #           'Columns': [0]}
